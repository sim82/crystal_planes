...
pub struct Blocklist {
    single: Vec<(u32, f32)>,
    vec2_ff: Vec<[f32; 2]>, // keep simd vectors densely packed
    vec4_ff: Vec<[f32; 4]>,
    vec8_ff: Vec<[f32; 8]>,
    vec16_ff: Vec<[f32; 16]>,
    vec2: Vec<u32>,
...
fn from_slice16(v: &[f32]) -> [f32; 16] {
    let mut array = [0f32; 16];
    let src = &v[..array.len()]; // panics if not enough data
    array.copy_from_slice(src);
    array
}
fn from_slice8(v: &[f32]) -> [f32; 8] {
    let mut array = [0f32; 8];
    let src = &v[..array.len()]; // panics if not enough data
    array.copy_from_slice(src);
    array
}
fn from_slice4(v: &[f32]) -> [f32; 4] {
    let mut array = [0f32; 4];
    let src = &v[..array.len()]; // panics if not enough data
    array.copy_from_slice(src);
    array
}
fn from_slice2(v: &[f32]) -> [f32; 2] {
            ...
            .flat_map(|x| x.split_aligned(&[16, 8, 4, 2, 1]))
        {
            match ext.ffs.len() {
                16 => {
                    ...
                    vec16_ff.push(from_slice16(&ext.ffs[..]));
                }
                8 => {
                    vec8.push(ext.start);
                    vec8_ff.push(from_slice8(&ext.ffs[..]));
                }
                4 => {
                    vec4.push(ext.start);
                    vec4_ff.push(from_slice4(&ext.ffs[..]));
                }
                2 => {
                    vec2.push(ext.start);
                    vec2_ff.push(from_slice2(&ext.ffs[..]));
                }
...
impl RadWorkblockSimd<'_> {
    ...
    pub fn do_iter(&mut self) -> usize {
        ...
        for (i, ff_i) in self.blocks.iter().enumerate() {
            ...
            let vdiffuse_r = [diffuse.x; 2];
            let vdiffuse_g = [diffuse.y; 2];
            let vdiffuse_b = [diffuse.z; 2];
            let mut vsum_r = [0f32; 2];
            let mut vsum_g = [0f32; 2];
            let mut vsum_b = [0f32; 2];

            for (j, ff) in ff_i.vec2.iter().zip(&ff_i.vec2_ff) {
                let j = *j as usize;
                let vr = from_slice2(&r[j..j + 2]);
            ...
            rad_g += vsum_g.iter().sum::<f32>();
            rad_b += vsum_b.iter().sum::<f32>();

            let vdiffuse_r = [diffuse.x; 4];
            let vdiffuse_g = [diffuse.y; 4];
            let vdiffuse_b = [diffuse.z; 4];
            let mut vsum_r = [0f32; 4];
            let mut vsum_g = [0f32; 4];
            let mut vsum_b = [0f32; 4];

            for (j, ff) in ff_i.vec4.iter().zip(&ff_i.vec4_ff) {
                let j = *j as usize;
                let vr = from_slice4(&r[j..j + 4]);
            ...
            let vdiffuse_r = [diffuse.x; 8];
            let vdiffuse_g = [diffuse.y; 8];
            let vdiffuse_b = [diffuse.z; 8];
            let mut vsum_r = [0f32; 8];
            let mut vsum_g = [0f32; 8];
            let mut vsum_b = [0f32; 8];

            for (j, ff) in ff_i.vec8.iter().zip(&ff_i.vec8_ff) {
                let j = *j as usize;
                let vr = from_slice8(&r[j..j + 8]);
            ...
            let vdiffuse_r = [diffuse.x; 16];
            let vdiffuse_g = [diffuse.y; 16];
            let vdiffuse_b = [diffuse.z; 16];
            let mut vsum_r = [0f32; 16];
            let mut vsum_g = [0f32; 16];
            let mut vsum_b = [0f32; 16];

            for (j, ff) in ff_i.vec16.iter().zip(&ff_i.vec16_ff) {
                let j = *j as usize;
                let vr = from_slice16(&r[j..j + 16]);
